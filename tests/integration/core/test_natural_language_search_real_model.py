...
class _NullTextSearchIndex:
    """Not exercised by semantic-mode queries; satisfies the constructor."""
...
async def env(tmp_path: Path) -> AsyncIterator[_Env]:
    db_path = tmp_path / "nl_search_real.db"
...
async def test_natural_language_query_returns_the_matching_photo_in_the_top_five(
    env: _Env,
) -> None:
    ...
    for query_text, expected_photo in (
        ("a photo of the color red", red),
        ("a photo of the color blue", blue),
        ("a photo of the color green", green),
    ):
        result = await env.search_service.search(
            SearchQuery(mode="semantic", text=query_text, limit=5)
        )
        top_five_ids = [r.photo_id for r in result.results[:5]]
        assert expected_photo.id in top_five_ids
        # with only 3 labeled photos, a well-matched query should rank top-1
        assert top_five_ids[0] == expected_photo.id
